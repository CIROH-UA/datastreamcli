...
def check_forcings(
    ...
    assert start_time == forcings_start, (
        f"Realization start time {start_time} does not match forcing start time {forcings_start}"
    )
    assert end_time == forcings_end, (
        f"Realization end time {end_time} does not match forcing end time {forcings_end}"
    )
    assert dt_s == dt_forcings_s, (
        f"Realization output_interval {dt_s} does not match forcing time axis {dt_forcings_s}"
    )
    ...
    for jval in validations:
        ...
        if jval == "forcing":
            if files[0].endswith(".nc"):
                ...
                with xr.open_dataset(
                    os.path.join(forcing_dir, nc_file)
                ) as ngen_forcings:
                    df = ngen_forcings["precip_rate"]
                    forcings_start = datetime.fromtimestamp(
                        ngen_forcings.Time.values[0, 0], timezone.utc
                    )
                    forcings_end = datetime.fromtimestamp(
                        ngen_forcings.Time.values[0, -1], timezone.utc
                    )
                    check_forcings(
                        serialized_realization,
                        forcings_start,
                        forcings_end,
                        len(ngen_forcings.time.values),
                    )
                    continue
        ...
        for j, jcatch in enumerate(catchments):
            ...
            if not bool(compiled.match(jfile)):
                raise Exception(
                    f"{jcatch} -> File {jfile} does not match pattern specified {pattern}"
                )
            ...
            if jval == "forcing":
                if j == 0:
                    ...
                    forcings_start = datetime.strptime(
                        df["time"].iloc[0], "%Y-%m-%d %H:%M:%S"
                    )
...
def validate_data_dir(
    data_dir: str, troute_restart: str = "", troute_crosswalk: str = ""
) -> None:
    """
    ...
    if "config/troute.yaml" not in str(
        ...
        forcing_dir = os.path.join(
            relative_dir, serialized_realization.global_config.forcing.path
        )
        ...
        serialized_realization = NgenRealization.parse_file(realization_file)
        serialized_realization.time.start_time = (
            serialized_realization.time.start_time.replace(tzinfo=timezone.utc)
        )
        serialized_realization.time.end_time = (
            serialized_realization.time.end_time.replace(tzinfo=timezone.utc)
        )
        for jform in serialized_realization.global_config.formulations:
            for jmod in jform.params.modules:
                if jmod.params.model_name == "SLOTH":
                    continue
                jdir = jdir_dict[jmod.params.model_name]
                jconfig_dir = os.path.join(config_dir, jdir)
                config_files = [
                    os.path.join(f"config/cat_config/{jdir}", x)
                    for x in [x for _, _, x in os.walk(jconfig_dir)][0]
                ]
                pattern = str(jmod.params.config)
                jcatch_pattern = pattern.replace("{{id}}", r"[^/]+")
                compiled = re.compile(jcatch_pattern)
                validate_files[jmod.params.model_name] = {
                    "pattern": pattern,
                    "files": sorted(
                        [x for x in config_files if bool(compiled.match(x))]
                    ),
                }
...
if __name__ == "__main__":
    ...
    if args.data_dir:
        ...
        raise Exception(
            "Must specify either data folder path or tarball path, not both."
        )
    else:
    ...
    validate_data_dir(
        data_dir, troute_restart=troute_restart, troute_crosswalk=troute_crosswalk
    )

    if ii_delete_folder:
        os.system("rm -rf /tmp/ngen_data_dir")
