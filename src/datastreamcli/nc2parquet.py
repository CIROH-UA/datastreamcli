...
def nc2parquet(nc_file: str, out_dir: str) -> None:
    ...
    for idx, file_path in enumerate(nc_files, 1):
        ...
        df.to_parquet(output_file, compression="brotli", index=False, engine="pyarrow")
...
Examples:
  ...
  nc2parquet --nc_file /path/to/climate_data.nc --out_dir /path/to/parquet_files
    ...
    parser.add_argument("--nc_file", required=True, help="Path to input NetCDF file")

    parser.add_argument(
        "--out_dir", required=True, help="Directory to write output Parquet file"
    )
