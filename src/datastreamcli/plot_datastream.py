...
def get_ec2_instance_price(instance_type):
    ...
    for page in response_iterator:
        for product in page["PriceList"]:
            ...
            if (
                "instanceType" in product_data["product"]["attributes"]
                and product_data["product"]["attributes"]["instanceType"]
                == instance_type
            ):
                price_dimension = product_data["terms"]["OnDemand"][
                    next(iter(product_data["terms"]["OnDemand"]))
                ]["priceDimensions"]
                ...
                try:
                    ...
                    if instance_type == "r7g.2xlarge":
                        ...
                        raise Exception(
                            f"failure to locate price for instance {instance_type}"
                        )
...
def profile_txt2df(txt_file):
    ...
    for jline in data:
        ...
        if step in out_dict.keys():
            ...
            out_dict[step]["duration_seconds"] = (
                out_dict[step]["end_time"] - out_dict[step]["start_time"]
            ).seconds
    ...
    for jstep in out_dict_tmp:
        if "end_time" not in out_dict[jstep]:
            out_dict.pop(jstep)
    ...
    for profile_name in profile_dict:
        ...
        for j, jstep in enumerate(steps):
            ...
            if jstep not in loop_dict:
                loop_dict[jstep] = pd.DataFrame(
                    {"profile": [profile_name], "duration_minutes": [jstep_duration]}
                )
            else:
                loop_dict[jstep] = pd.concat(
                    [
                        pd.DataFrame(
                            {
                                "profile": [profile_name],
                                "duration_minutes": [jstep_duration],
                            }
                        ),
                        step_dfs[jstep],
                    ],
...
def plot_group(profile_dict, conf_dict, input_csv):
    ...
    for jdf in step_dfs:
        ...
        if jdf != "total_runtime":
            step_dfs_proportion[jdf]["proportion"] = 100 * (
                step_dfs[jdf]["duration_minutes"].reindex(order)
                / step_dfs["total_runtime"]["duration_minutes"].reindex(order)
            )
        step_dfs_proportion[jdf] = step_dfs_proportion[jdf].drop(
    ...
    write_to_csv(
        input_csv, ncatchment_list_sorted, step_dfs_sorted, conf_dict, fp_step_dfs
    )
    ...
    plot_bar_chart(
        ncatchment_list_sorted,
        step_dfs_proportion,
        "ngen-datastream profiling",
        "profile_vpu_propotions.png",
        "Proportion Total Runtime",
        "proportion",
        add_text,
        colors,
    )
...
def write_to_csv(benchmark, catchments, dfs, confs, fp_dfs):
    ...
    diff = datetime.strptime(
        confs["01"]["globals"]["end_date"], "%Y%m%d%H%M"
    ) - datetime.strptime(confs["01"]["globals"]["start_date"], "%Y%m%d%H%M")
    nts = diff.days * 24
    ...
    for j, jrun in enumerate(dfs["GET_RESOURCES"].profile):
        runtime_min = dfs["total_runtime"].set_index("profile")["duration_minutes"][
            jrun
        ]
        jdict = {}
        jdict["Run ID"] = last + j
        ...
        jdict["Catchments/core/second"] = (
            catchments[j] / confs[jrun]["host"]["host_cores"] / (runtime_min / 60)
        )
        jdict["CONUS equivalent concurrent vCPUs"] = (
            len(ncatchment_vpu) * confs[jrun]["host"]["host_cores"]
        )
    ...
    benchmark_out = os.path.join(
        os.path.dirname(benchmark), f"{benchmark_name}_{today}.xlsx"
    )
    ...
    runtime_min = (
        fp_dfs["total_runtime"].set_index("profile")["duration_minutes"].iloc[0]
    )
    jdict = {}
    jdict["Run ID"] = last + j
    jdict["Provider"] = "AWS"
    ...
    for jstep in steps:
        jdict[f"{jstep} Cost"] = (
            combined_df_fp["duration_minutes"][jstep] * cost_per_hr / 60
        )
    jdict["Cost/hr"] = f"${cost_per_hr}"
    ...
    with pd.ExcelWriter(benchmark_out) as writer:
        fp_df_append.to_excel(
            writer, sheet_name=datetime.now().strftime("%Y%m%d") + "_fp"
        )
...
def plot_scaling(xticklabelz, dfs, title, save_name, y_label, key, add_text, colors):
    ...
    for j, jcol in enumerate(combined_df[key].columns):
        ...
        slope, intercept, r_value, p_value, std_err = linregress(
            xticklabelz, combined_df[key][jcol]
        )
        r_squared = r_value**2
    ...
    plt.legend(
        legend_entries,
        fontsize="small",
        title="(slope 10k catchments/minute), R^2",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_dir",
        help="Path to directory holding many datastream-configs/",
        default=None,
    )
