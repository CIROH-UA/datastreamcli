...
def create_conf_nwm(args):
    ...
    if "RETRO" in args.forcing_source:
        ...
        nwm_conf = {
            ...
            "selected_var_types": selected_var_types,
            "write_to_file": True,
        }
    else:
        if "CHRT" in args.forcing_source:
        ...
        if "NOMADS" in args.forcing_source:
            ...
            return (
                ...
                end_str_real,
                ens_member,
            )  # nextgen forcings have been supplied directly
        else:
        ...
        if "SHORT_RANGE" in args.forcing_source:
            ...
            # Adjust hours
            start_str_real = datetime.strftime(
                datetime.strptime(start_str_real, "%Y-%m-%d %H:%M:%S")
                + timedelta(hours=fcst_cycle),
                "%Y-%m-%d %H:%M:%S",
            )
            end_str_real = datetime.strftime(
                datetime.strptime(end_str_real, "%Y-%m-%d %H:%M:%S")
                + timedelta(hours=fcst_cycle + num_hrs - 1),
                "%Y-%m-%d %H:%M:%S",
            )
            ...
            end_str_real = datetime.strftime(
                datetime.strptime(end_str_real, "%Y-%m-%d %H:%M:%S")
                + timedelta(hours=fcst_cycle + num_hrs - 1 - cut_off),
                "%Y-%m-%d %H:%M:%S",
            )
            ...
            if "EXTEND" in args.forcing_source:
                ...
                "RESTART" in args.forcing_source
            ):  # don't need the 3 hr lookback for a restart file
                runinput = 5
                num_hrs = 1
                dt = 0
                fcst_cycle = int(args.forcing_source[-2:])
        ...
        if (
            "DAILY" in start
            and start_dt_exact.hour < fcst_cycle
            and args.end_date == ""
        ):
            start_str_real = datetime.strftime(
                datetime.strptime(start_str_real, "%Y-%m-%d %H:%M:%S")
            ...
            start_str_nwm = datetime.strftime(
                datetime.strptime(start_str_nwm, "%Y%m%d%H%M") - timedelta(days=1),
                "%Y%m%d%H%M",
            )
            end_str_nwm = datetime.strftime(
                datetime.strptime(end_str_nwm, "%Y%m%d%H%M") - timedelta(days=1),
                "%Y%m%d%H%M",
            )
...
def create_conf_fp(args, start_real):
    ...
    if len(args.s3_bucket) > 0:
        if "DAILY" in args.start_date:
            args.s3_prefix = re.sub(
                r"\DAILY",
                datetime.strptime(start_real, "%Y-%m-%d %H:%M:%S").strftime("%Y%m%d"),
                args.s3_prefix,
            )
...
def create_confs(args):
    ...
    if not os.path.exists(ngen_config_dir):
        os.system(f"mkdir -p {ngen_config_dir}")
    ...
    config_class2envs(
        args, filename=os.path.join(datastream_meta_dir, "datastream.env")
    )

    print(
        f"datastream metadata have been generated and placed here\n{datastream_meta_dir}"
    )
    ...
    if args.forcings.endswith(".tar.gz"):
        ...
        raise Exception(
            f"Forcing file {args.forcings} not understood, must be .nc or .tar.gz"
        )

    # routing-only changes, points forcing path to a dummy file to appease sloth
    if "CHRT" in args.forcing_source and "RESTART" not in args.forcing_source:
        if "file_pattern" in forcing_dict:
            del forcing_dict["file_pattern"]
        if "provider" in forcing_dict:
